...
def evaluate_room_state(snapshot: RoomSnapshot) -> str:
    ...
    if snapshot.room_id == "room_alpha" and snapshot.sleeping:
...
def advance_room_state(current_state: str, snapshot: RoomSnapshot) -> str:
    ...
    if current_state == "sleeping" and snapshot.room_id == "room_alpha" and snapshot.bed_motion_only:
        return "bed_motion_only"
    if current_state == "bed_motion_only" and snapshot.room_id == "room_alpha" and snapshot.sleeping:
        return "sleeping"
    return next_state
