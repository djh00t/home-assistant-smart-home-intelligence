Feature: Face enrollment and match canonicalization
  ...
  Scenario: Canonical face enrollment record captures identity and context
    Given a raw face enrollment with person "sel", room "bedroom_spare" and camera "cam_lounge_room_front"
    And face signature "sig_abc123"
    When enrollment is normalized
    Then the enrollment record should preserve person_id "sel"
    And the enrollment room should be "bedroom_spare"
    And the enrollment camera should be "cam_lounge_room_front"
    And the retention days should be "90"

  Scenario: Canonical face match event is emitted with stable source and confidence
    Given a face match for person "sel" in room "bedroom_spare" from camera "cam_lounge_room_front"
    ...
    And the event should keep room "bedroom_spare"
    And the event should keep camera "cam_lounge_room_front"
