...
def validate_module_behavior() -> None:
    ...
    if evaluate_room_state(RoomSnapshot("hall", 1, 0)) != "humans_only":
    ...
    if evaluate_room_state(RoomSnapshot("living_room", 1, 1)) != "mixed":
        raise SystemExit("mixed state failed")
    if evaluate_room_state(RoomSnapshot("master_bedroom", 1, 0, sleeping=True)) != "sleeping":
        raise SystemExit("sleeping state failed")
    if evaluate_room_state(RoomSnapshot("master_bedroom", 1, 0, sleeping=True, bed_motion_only=True)) != "bed_motion_only":
        raise SystemExit("bed_motion_only state failed")
    if advance_room_state(
        "sleeping", RoomSnapshot("master_bedroom", 1, 0, sleeping=True, bed_motion_only=True)
    ) != "bed_motion_only":
